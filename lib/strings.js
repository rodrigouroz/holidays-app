...
export default function phraseMatches(phraseToMatch, phrase, threshold = 0.8) {
  let searchWhat = slugify(phraseToMatch.trim().toLowerCase());
  let searchIn = slugify(phrase.trim().toLowerCase());

  if (searchWhat == searchIn) {
    return true;
  }

  if (searchWhat.split(' ').length > searchIn.split(' ').length) {
    const tmp = searchWhat;
    searchWhat = searchIn;
    searchIn = tmp;
  }
