...
export async function getHolidays(countryCode, startDate, endDate = null, count = null) {
    console.debug(`Fetching for ${countryCode}. Start Date ${startDate}. End Date ${endDate}. Count ${count}`);
...
export async function getHolidaysWorldwide() {
    const response = await fetchAPI(`https://date.nager.at/api/v2/nextpublicholidaysworldwide`)
    
    const holidaysWorldwide = await response.filter(entry => entry.global == true)
