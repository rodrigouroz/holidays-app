...
describe('questionResolver', () => {
  ...
  test('gets the holidays by direct search', async () => {
    ...
    expect(moment).toHaveBeenCalled();
    expect(getHolidays).toHaveBeenCalledWith('US', today, undefined, undefined);
    expect(response).toEqual({
      status: 'success',
      answer_title: 'The holiday in United States is',
      holidays,
  ...
  test('gets the holidays by searching through the captured search in holidayBeta', async () => {
    const holidays = [
      ...
      {
        date: '2020-08-17',
        name: 'a holiday about something else',
        country: { name: 'United States' },
      },
    ];
    const today = '2020-08-05';
    ...
    expect(moment).toHaveBeenCalled();
    expect(getHolidays).toHaveBeenCalledWith('AR', today, undefined, undefined);
    expect(response).toEqual({
      status: 'success',
